...
/// Generated class from Pigeon that represents data sent in messages.
struct HSBeaconSession {
  var attributes: [AnyHashable: Any?]? = nil

  static func fromList(_ list: [Any?]) -> HSBeaconSession? {
    let attributes: [AnyHashable: Any?]? = nilOrValue(list[0])
...
class HelpScoutBeaconApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: HelpScoutBeaconApi?) {
    ...
    let addSessionChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.help_scout_beacon.HelpScoutBeaconApi.addSession", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      addSessionChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let sessionArg = args[0] as! HSBeaconSession
        do {
          try api.addSession(session: sessionArg)
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
